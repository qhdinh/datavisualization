...
void setup(){
  ...
  int i,j;
  for(i=0;i<23;i++){
    Country c = new Country(datasets.counname[i].name,datasets.getCouncode(datasets.counname[i].name));
    counsele[i]=c;
  }
  for(i=0;i<44;i++){
    Criteria cri = new Criteria(datasets.criteria[i].name,datasets.getCricode(datasets.criteria[i].name));
    crisele[i]=cri;
  }
...
void drawline(){
  ...
  for(i=0;i<counnum;i++){
    ...
    line(10,10+i*10,25,10+i*10);
    text(counsele[i].name,30,10+i*10+5);
  }
...
void drawsearchline(String str){
  ...
  for(j=0;j<crinum;j++){
    ...
    if(j!=crinum-1){
      ...
      s=(float)clines[datasets.getCouncode(str)].value[0];
      ss=str(s);
      if(ss.equals("NaN")){
      continue;
      }
      else line(clines[datasets.getCouncode(str)].xspot[j],clines[datasets.getCouncode(str)].yspot[j],clines[datasets.getCouncode(str)].xspot[0],clines[datasets.getCouncode(str)].yspot[0]);
